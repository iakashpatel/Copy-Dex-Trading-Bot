...
async function performTx(pendingTx, path) {
  ...
  const amt = ethers.toBigInt(
    getValue(config.thresholdAmt, getValue(config.tokens, path[0], 0), 1)
  );
  ...
  const txObj = {
    ...
    maxPriorityFeePerGas:
      ethers.toBigInt(pendingTx.maxPriorityFeePerGas) + ethers.toBigInt(20),
...
function getValue(array, key, byName) {
  ...
  for (let i = 0; i < array.length; i++) {
    if (byName) {
      if (
        array[i].name.toString().toLowerCase() == key.toString().toLowerCase()
      ) {
      ...
      if (
        array[i].value.toString().toLowerCase() == key.toString().toLowerCase()
      ) {
...
function validateParams(path) {
  if (
    getValue(config.tokens, path[0], 0) &&
    getValue(config.tokens, path[1], 0)
  ) {
    return true;
  } else {
    return false;
...
async function validateTokenBalance(tokenAddress) {
  ...
  if (
    parseInt(balance) >
    getValue(config.thresholdAmt, getValue(config.tokens, tokenAddress, 0), 1)
  ) {
    return true;
  } else {
    return false;
...
async function createPayload(path) {
  ...
  const payload = interface.encodeFunctionData("swap", [
...
function sortTokens(token) {
  if (
    ethers.toBigInt(token[0].toString()) < ethers.toBigInt(token[1].toString())
  ) {
...
fs.writeFileSync("Logs.txt", "Started listening to mempool........", {
  flag: "a",
});
